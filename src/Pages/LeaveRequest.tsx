...
//       (approver: any) => approver.EmailId.toLowerCase() === email.toLowerCase()
//     );
//   };
...
export default function Leave() {
  ...
  const isUserApprover = async () => {
    try {
      ...
      const approvers = await getApproverList();
      console.log("Approver List:", approvers);

      const isApprover = approvers.some((approver: any) => {
        const isMatch = approver.EmailId.toLowerCase() === email.toLowerCase();
        console.log(
          `Checking if ${email.toLowerCase()} matches ${approver.EmailId.toLowerCase()}:`,
          isMatch
        );
        return isMatch;
      });
  ...
  const isUserHRAdmin = async () => {
    try {
      const email = await getCurrentUserEmail();
      console.log("Current User Email:", email);
      const approvers = await getHRList();
      console.log("Approver List:", approvers);
  ...
  useEffect(() => {
    async function checkUserRoles() {
      try {
        const approver = await isUserApprover();
        const hrAdmin = await isUserHRAdmin();
        setIsApprover(approver);
